...
数列的极限不存在只有两种情况：
1. 数列发散，即 $\lim_{ n \to \infty } \alpha_{n}=\infty$。
2. 数列在常数间震荡。

---

> **Definition**: 数列的聚点
> 在某点附近无论多么小的范围内，都有无限个项，数列$\{\alpha_{n}\}$，给定任意正实数$\varepsilon$， 如果满足 $|\alpha_{n}-\beta|<\varepsilon$ 的项有无穷个，则 $\beta$ 是 $\{\alpha_{n}\}$ 的一个聚点，如果数列存在极限，则聚点有且只有一个，为该数列的极限（证明见下文）。

与极限的关系：聚点只要求有无穷个项在该点附近，一个数列可能会有多个极限点。

---


> **Theorem 1.1.1**
> 数列$\{\alpha_{n}\}$ 收敛于 $\alpha$ 的充分必要条件是，任意给定满足$\rho<\alpha<\sigma$的 $\rho,\sigma$，除了有限c个正整数 $n$ 以外（即除了$n\leq n_{0}(\varepsilon)$ 的部分） $\rho<\alpha_{n}<\sigma$ 都成立。
...
设$A^\prime$，为$A$的补集，即为使得 “有无限个项满足 $\alpha_{n}\leq \sigma$”，的所有 $\sigma$ 的集合。
此时，$\langle A,A^\prime \rangle$ 是 $\mathbb{R}$ 的分划，显然 $\rho<\sigma$。


证明 $A$ 与 $A^\prime$ 非空：取一正整数 $l >n_{0}(1)$，则只要 $n>n_{0}(1)$ 时，都有 $a_{l}-1<a_{n}<a_{l}+1$ ，因此 $a_{l}-1 \in A$，$a_{l}+1 \in B$ ，即 $A$ 与 $A^\prime$ 非空。
...
按照我们的假设，满足 $\alpha_{n}\leq\rho$ 的项有有限个，即 **$\rho<\alpha_{n}$ 除了有限个项外都成立**。
由于有理数的稠密性，选取一个 $r$ 满足 $\alpha<r<\sigma$，令$\varepsilon=\sigma-r$。

根据条件，对于 $\varepsilon$ 存在 $n_{0}(\varepsilon)$，使得 $n>n_{0}(\varepsilon),m>n_{0}(\varepsilon)$ 时，$|\alpha_{n}-\alpha_{m}|<\varepsilon =\sigma-r$，成立。又因为 $r>\alpha$ 所以 $r \in A^\prime$，因此，有无数个正整数 $m$ 使得 $\alpha_{m}\leq r$ 成立，如果确定了其中一个 $m$ 且 $m>n_{0}(\varepsilon)$ ，则只要 $n>n_{0}(\varepsilon)$ 时，就有 $\alpha_{n}-\alpha_{m}<\sigma-r$ 又因为 $\alpha_{m}\leq r$ 所以 $\alpha_{n}<\sigma$ ，有无限个 $n$ 使得 $\alpha_{n}<\sigma$ 成立，即 **$\alpha_{n}<\sigma$ 除了有限个项外都成立**。
...
证明：
设 $s_{n} =\sum^n_{k=1}(-1)^{n+1}\alpha_{n}$ 则：
$$
\begin{aligned}
s_{2n}=(a_{1}-a_{2})+(a_{3}-a_{4})-a_{5})+\dots+(a_{2n-1}-a_{2n}) \\
